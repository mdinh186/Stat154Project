...
mrg_combine = function(dat){
  dat <- data.table(dat)
   dat$`martial-status` = as.character(dat$`martial-status`)
   mrg = unique(dat$`martial-status`)
...
#Constructing CSVs and Dataframes
Construct df_feat:
```{r}
df_feat =  combine_education(df)
names(df_feat) <- c("age", "workclass", "fnlwgt", "education", "education-num", 
...
quant <- quantile(df_feat$`hours-per-week`,seq(0,1,.10))
...
Construct df_impute:
```{r}
df_impute =  combine_education(dataMissForestImputed)
names(df_impute) <- c("age", "workclass", "fnlwgt", "education", "education-num", 
              "marital_status", "occupation", "relationship", "race", "sex",
              "capital-gain", "capital-loss", "hours-per-week",
              "native_country","income")
df_impute = education_median(df_impute)
df_impute$`education-num` <- NULL
df_impute <- age_combine(df_impute)
df_impute = race_combine(df_impute) 
df_impute <- marital_combine(df_impute)
df_impute <- occupation_combine(df_impute)
df_impute <- country_combine(df_impute)
df_impute <- workclass_combine(df_impute)
df_impute <- relation_combine(df_impute)
df_impute <- cap_gain_clean_outlier(df_impute)

quant <- quantile(df_impute$`hours-per-week`,seq(0,1,.10))
df_impute <- hours_combine(df_impute)
df_impute <- Income_factor(df_impute)
df_impute <- factorize(df_impute)
```

Construct df_remove:
```{r}
df_remove =  combine_education(dataNoNA)
names(df_remove) <- c("age", "workclass", "fnlwgt", "education", "education-num", 
              "marital_status", "occupation", "relationship", "race", "sex",
              "capital-gain", "capital-loss", "hours-per-week",
              "native_country","income")
df_remove = education_median(df_remove)
df_remove$`education-num` <- NULL
df_remove <- age_combine(df_remove)
df_remove = race_combine(df_remove) 
df_remove <- marital_combine(df_remove)
df_remove <- occupation_combine(df_remove)
df_remove <- country_combine(df_remove)
df_remove <- workclass_combine(df_remove)
df_remove <- relation_combine(df_remove)
df_remove <- cap_gain_clean_outlier(df_remove)

quant <- quantile(df_remove$`hours-per-week`,seq(0,1,.10))
df_remove <- hours_combine(df_remove)
df_remove <- Income_factor(df_remove)
df_remove <- factorize(df_remove)
```

Making CSV files of our datasets
```{r}
#Exporting as a CSV
write.csv(df_feat, "data/EDA_feat_OG.csv", row.names = FALSE)
write.csv(df, "data/EDA_OG.csv", row.names = FALSE)
write.csv(df_impute, "data/EDA_feat_imp.csv", row.names=FALSE)
write.csv(df_remove, "data/EDA_feat_rem.csv", row.names = FALSE)
